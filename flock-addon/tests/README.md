...
Two offline test suites guard the addon against regression. Both run on a
laptop without any cluster and are wired into `make verify` so every
change goes through them before deployment.
...
* **No cluster e2e yet.** A future kind+OCM e2e would extend the matrix
  ...
  rollout. It is intentionally out of `make verify` because the full
  fixture takes minutes to bring up; `make test-e2e` is reserved for it.

## CI hook

`make verify` chains `lint → template → test-unit → test-integration`,
so the existing CI invocation already runs both suites without further
configuration. `make test-chart` (called by addon-contrib's CI for chart
validation) runs `lint + test-unit`, which keeps the chart-only path
fast while still catching template regressions.
